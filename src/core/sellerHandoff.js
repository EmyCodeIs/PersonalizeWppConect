...
async function getAutomationBlock(channel, clientId) {
  ...
  if (sellerAssignment.assigned) {
    Store.setHumanControlBlock(clientId, {
      reason: 'seller_label',
      source: 'seller_label',
  ...
  const humanControl = Store.getHumanControlState(clientId);
  if (humanControl?.blocked) {
    return {
      blocked: true,
      reason: humanControl.reason || 'human_block',
      seller: humanControl.seller || null,
      labelName: humanControl.labelName || null,
      source: humanControl.source || 'profile',
      details: humanControl,
    };
  }
